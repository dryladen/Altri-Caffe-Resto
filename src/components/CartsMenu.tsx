  ...
  if (carts.length === 0) {
    return (
      <div className="flex w-full flex-col items-center justify-center h-screen p-4">
        ...
        <Link href="/">
          <Button className="mt-4">Kembali Belanja</Button>
        </Link>
      </div>
    );
  } else {
    ...
    return (
      <>
        {children}
        <div className="flex w-full relative flex-col gap-4 px-4 grow py-4">
          ...
          <Link
            className="flex rounded-full mt-4 sticky gap-2 justify-center right-4 left-4 bottom-4 shadow-md bg-primary z-50 py-1"
            href="/checkout"
          >
            <div className="flex text-white gap-2 items-center px-4 py-2 w-full">
              <ShoppingCart size={18} strokeWidth={3} />
              <div className="flex items-center">
                <span className="font-bold p-0 text-sm">
                  {carts.length} Item
                </span>
